...
JSON/CSV, сохранение в SQLite и проверки доступа. Подключение LLM поддерживается
через локальную Ollama; без модели интерфейс честно показывает многофакторный
подбор. Облачные адаптеры OpenAI/NVIDIA предусмотрены, но передача данных
выключена по умолчанию и для датасета организаторов не используется.
...
### Локальная модель — режим для данных хакатона

Установить [Ollama](https://ollama.com/download), затем:
...
Ollama должна работать на `http://127.0.0.1:11434`. При необходимости запустить
`ollama serve` в отдельном терминале. Перезапустить приложение после изменения `.env`.
Модель можно заменить на другую установленную instruction-модель с JSON-выводом.
...
Холодный запуск локальной модели может превысить лимит — перед защитой модель
следует прогреть и проверить на ноутбуке команды.

### Облачные адаптеры

Для собственных данных, для которых разрешена передача провайдеру, предусмотрены
`AI_PROVIDER=openai` с `OPENAI_API_KEY` либо `AI_PROVIDER=nvidia` с `NVIDIA_API_KEY`.
Дополнительно требуется явный `CQ_ALLOW_CLOUD_DATA=true`. **В режиме хакатонного
датасета этот флаг оставляется false.** Наличие ключа само по себе передачу не включает.
...
flowchart LR
    ...
    ENGINE --> AI[Локальная LLM / проверенный fallback]
    API --> DB[(SQLite)]
