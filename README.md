...
На Windows можно дважды щёлкнуть **`start-ai-demo.cmd`**. Откроется **http://127.0.0.1:8001**. Войдите сотрудником `E0001` с паролем `quest-demo`, дождитесь ответа в блоке «Рекомендации». Статус «Подбор AI» означает успешный ответ модели; провайдер, время ответа и сведения о кэше доступны в «О подборе». Кнопка «Обновить» повторяет подбор. Наличие ключа ещё не доказывает успешный запрос.
...
Скрипт создаёт временный набор из `examples/demo.py`, запускает отдельный локальный HTTP-сервер, входит сотрудником, вызывает `/api/ai`, проверяет реальный ответ модели, завершение активности с сохранением прогресса 50% → 70% и сброс к 50%. Исходный датасет и база приложения не читаются; `.env` и `CQ_ALLOW_CLOUD_DATA` не меняются. Для собственных временных примеров действует то же серверное разрешение, что в `--demo-ai`. Требуются ключ и доступ к выбранной модели.
...
| `examples/demo.py` | Независимый набор: 3 профиля, 6 событий, 3 навыка и 3 записи истории |
