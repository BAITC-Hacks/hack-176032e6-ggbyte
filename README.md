...
На Windows можно дважды щёлкнуть **`start-ai-demo.cmd`**. Откроется **http://127.0.0.1:8001**. Войдите сотрудником `E0001` с паролем `quest-demo`, дождитесь ответа в блоке «AI-навигатор: следующий шаг». Наличие ключа ещё не доказывает успешный запрос: проверьте в этом блоке подтверждение выбора моделью.
