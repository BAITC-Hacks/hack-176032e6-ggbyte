...
function shell(body) {
  ...
  app.innerHTML = `<div class="shell"><aside class="sidebar"><div class="brand"><img src="/favicon.svg" alt=""><span>GGBYTE SKILLS</span></div><nav class="nav" aria-label="Основная навигация">${nav.map(([key, label]) => `<button data-view="${key}" class="${state.view === key ? "active" : ""}" ${state.view === key ? 'aria-current="page"' : ""}>${icon(key)}${label}</button>`).join("")}</nav></aside><div class="content"><header class="topbar">${dataModeBanner()}<div class="account"><span class="avatar" aria-hidden="true">${isHR ? "HR" : esc(e?.full_name.split(" ").map((n) => n[0]).slice(0, 2).join(""))}</span><span class="account-name">${isHR ? "HR-кабинет" : esc(e?.full_name)}</span><button class="logout" id="logout">Выйти</button></div></header><main class="page">${body}</main></div></div>`;
